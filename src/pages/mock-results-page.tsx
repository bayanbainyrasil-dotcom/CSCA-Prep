...
import { ServerMockResults } from '@/features/mock/server-mock-results';

/**
 * `?attempt=<attemptId>` reviews a server-graded attempt. Without it the page
 * shows the local demo summary, which is computed in the browser and labelled
 * as such.
 */
export default function MockResultsPage() {
  ...
  if (attemptId && isServerMockAvailable()) {
    return <ServerMockResults attemptId={attemptId} />;
  }

  return <DemoMockResults />;
}

function DemoMockResults() {
  const { user } = useAuth();
