...
const settingsKey = (ownerId: string) => `csca-settings-ui-v1:${ownerId}`;
...
function initialSettings(user: ReturnType<typeof useAuth>['user']): SettingsState {
  ...
  return SettingsStateSchema.parse({
    targetDate: user?.targetDate ?? '2026-12-05',
  ...
  const exportData = async () => {
    ...
    try {
      ...
      if (functions && !isDemo) {
        ...
        data = { ...(await repository.exportValidatedData()), source: 'local-demo', settings };
      }
