...
export default function SettingsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (savedProfile) updateSessionProfile(savedProfile);
  ...
  return <div>
    ...
    <div className="content-grid"><div className="space-y-4 lg:col-span-8">
      ...
      <Card><CardContent className="p-5 sm:p-6"><h2 className="font-display text-xl font-semibold">Experience</h2><div className="mt-5 space-y-1">{([['sound', 'Sound effects', 'Subtle answer feedback'], ['animations', 'Animations', 'Fast page and progress motion'], ['reminders', 'Study reminders', 'Only after notification permission']] as const).map(([key, label, description]) => <div key={key} className="flex items-center gap-4 rounded-xl p-3 hover:bg-secondary/50"><div className="flex-1"><p className="text-sm font-semibold">{label}</p><p className="text-xs text-muted-foreground">{description}</p></div><Switch.Root aria-label={label} checked={settings[key]} onCheckedChange={(value) => update(key, value)} className="relative h-12 w-12 rounded-xl before:absolute before:inset-x-0 before:top-1/2 before:h-7 before:-translate-y-1/2 before:rounded-full before:bg-secondary before:content-[''] data-[state=checked]:before:bg-primary"><Switch.Thumb className="absolute left-0 top-1/2 block h-5 w-5 -translate-y-1/2 translate-x-1 rounded-full bg-white shadow transition-transform data-[state=checked]:translate-x-6" /></Switch.Root></div>)}</div></CardContent></Card>
